...
public class SalaryConfigPanel extends JPanel {
	ISalaryConfigDao salaryConfigDao = new ISalaryConfigDaoImpl();
	...
	public SalaryConfigPanel() {
		this.setName(ModuleName.SALARYCONFIG);
		init();
	}
	
	public void init() {
		final SalaryConfigPanel lp = this;
		cols.add("编号");
		cols.add("总达标奖");
		cols.add("税率");
		cols.add("起征点");
		...
		addBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				SalaryConfigDialog dialog = new SalaryConfigDialog();
		...
		editBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent paramActionEvent) {
				...
				paramMap.put("id", id);
				List<SalaryConfig> list = salaryConfigDao.select(paramMap);
				SalaryConfig salaryConfig = list.get(0);
				SalaryConfigDialog dialog = new SalaryConfigDialog();
				dialog.initDialog(salaryConfig, lp);
			}
		...
		delBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent paramActionEvent) {
				...
				if(value == JOptionPane.YES_OPTION) {
					boolean success = salaryConfigDao.delete(salaryConfig);
					if(success) {
						lp.initGrid();
						JOptionPane.showMessageDialog(lp, "删除成功", "温馨提示",JOptionPane.INFORMATION_MESSAGE);
						return;
	...
	public void initGrid() {
		try {
			row.clear();
        	List<SalaryConfig> salaryConfigs = salaryConfigDao.select(null);
        	for(int i = 0; i < salaryConfigs.size(); i ++) {
        		SalaryConfig salaryConfig = salaryConfigs.get(i);
        		Vector<String> newRow = new Vector<String> ();
				newRow.add(String.valueOf(salaryConfig.getId()));
				newRow.add(String.valueOf(salaryConfig.getTotalReachGoalBonus()/100.0));
				newRow.add(String.valueOf(salaryConfig.getTaxRate()));
				newRow.add(String.valueOf(salaryConfig.getTaxThreshold()/100.0));
				newRow.add(String.valueOf(salaryConfig.getSalaryStartDay()));
				newRow.add(String.valueOf(salaryConfig.getSalaryEndDay()));
				row.add(newRow);
        	}
        	model = new DefaultTableModel(row, cols);
