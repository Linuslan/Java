...
public class DepartmentPanel extends JPanel {
	...
	public void init() {
		...
		editBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent paramActionEvent) {
				...
				paramMap.put("id", id);
				List<Department> list = departmentDao.select(paramMap);
				Department department = list.get(0);
				DepartmentDialog dialog = new DepartmentDialog();
				dialog.initDialog(department, lp);
			}
	...
	public void initGrid() {
		try {
			row.clear();
        	List<Department> departments = departmentDao.select(null);
        	for(int i = 0; i < departments.size(); i ++) {
        		Department department = departments.get(i);
        		Vector<String> newRow = new Vector<String> ();
				newRow.add(String.valueOf(department.getId()));
				newRow.add(department.getName());
				newRow.add(department.getMemo());
				row.add(newRow);
        	}
        	model = new DefaultTableModel(row, cols);
